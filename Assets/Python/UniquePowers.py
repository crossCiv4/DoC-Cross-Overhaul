...
@handler("BeginGameTurn")
def mamlukPower(iGameTurn):
	# get one extra commerce in your capital for each city you own
	if player(iMamluks).isAlive():
		capital = player(iMamluks).getCapitalCity()
		if capital:
			capital.setBuildingCommerceChange(infos.building(iPalace).getBuildingClassType(), CommerceTypes.COMMERCE_GOLD, player(iMamluks).getNumCities())
