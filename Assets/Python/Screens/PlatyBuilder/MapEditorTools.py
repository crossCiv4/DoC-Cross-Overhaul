...
def exportCore(iPlayer):
	...
	tExceptions = None
	lExceptions = plots.rectangle(BL, TR).land().where(lambda p: not (p.isPeak() and location(p) not in sPeakExceptions) and not p.isPlayerCore(iPlayer))
	if lExceptions:
...
def getSettlerValue(plot, *args, **kwargs):
	...
	if plot.isWater():
		return 0
	elif plot.isPeak() and location(plot) in sPeakExceptions:
		return 0
